...
- **事实依据 2 失效**：`reader/page-frame.ts` 已随整页接管体系退役（见 ADR-0006），「B 站 main 节点就地搬进 `#boc-reading-inline-host`」的链路不存在了——阅读模式不再搬移任何 B 站节点，文中 `page-frame.js:188/:199-201` 的行号引用随之作古。light DOM 结论对 Digest 面板继续成立（ADR-0006 已同述）。
- **S3 字节数过时**：「后果」一节的面板 2,122 B / 阅读 9,651 B / 门控 20,512 B / 播放器 AI 1,727 B 是 S3 分层时刻的测量值，此后多轮结构改版（digest-only-ui 等）已改动各面体积，具体数字以构建产物为准；裁决理由（收益已被分层兑现、剩余收益理论性）不依赖这些数值。
- **正文面板引用过时**：「结论」表与「否决方案」引用的经典侧栏面板 `#boc-panel` 已随 digest-only-ui 删除——现行 UI 模板不再包含旧壳节点，对话/总结界面整体活在阅读视图 `#boc-reading-view` 内；表中「面板 `#boc-panel`、阅读视图 `#boc-reading-view` 等扩展自有子树」的表述应按「阅读视图 `#boc-reading-view` 等扩展自有子树」理解，不迁 shadow 的判定对它同样成立。
