...
// 与 tests/chat/chat-runtime-stream.test.js 互补：那边测生命周期与结构，
...
//             tests/ui/markdown-stream-invariants.test.js 的说明）
//   不变量 4：光标每帧重建后重新接回 tail 尾部
//   不变量 5：终态整渲染与流式渲染逐字节一致（done 后整渲染重放）
//
// 已由现有测试覆盖、不重复：
//   不变量 3（剥除先于切分）的结构性部分：tests/ui/markdown-split-tail.test.js
