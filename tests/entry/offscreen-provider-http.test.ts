...
// - 端口 provider-http-offscreen 被认领后，请求在 offscreen 里 fetch 并回执；
// - 代发请求在飞期间，ASR 任务终态不得自关文档（概览是分钟级非流式请求，
//   被自关吞掉的话回执永远不到）；
...
describe("offscreen 概览代发端口", () => {
  it("provider-http-offscreen 请求在 offscreen 里 fetch 并回执 { ok, status, body }", async () => {
    ...
    await vi.waitFor(() => expect(session.port.postMessage).toHaveBeenCalledTimes(1));
    expect(session.port.postMessage.mock.calls[0][0]).toEqual({ ok: true, status: 200, body: "ok" });
