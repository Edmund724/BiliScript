...
// openai baseUrl，正是这个重复。登记与修复依据（2026-09-23 实测）：
// - moonshot：官方文档登记 Anthropic Base URL = https://api.kimi.com/coding/，
//   endpoint 示例 https://api.kimi.com/coding/v1/messages
//   （https://www.kimi.com/code/docs/en/）；实测 /coding/v1/messages 返回 401、
//   /coding/nope 返回 404。
// - opencodego：pi-ai 目录中 anthropic-messages 组 baseUrl =
//   https://opencode.ai/zen/go，openai-completions / responses 组才是 /zen/go/v1；
//   实测 /zen/go/v1/messages 返回 ModelError、/zen/go/v1/nope 返回 404。
// - stepfun：官方文档「推理模型接入」明示 Anthropic 协议路径
//   https://api.stepfun.com/step_plan/v1/messages，并警告 Anthropic SDK 的
//   base_url 应设为 https://api.stepfun.com/step_plan（不带 /v1）——与本仓
//   adapter 的拼接约定一致
...
const ANTHROPIC_PRESET_IDS = ["deepseek", "qwen", "zhipu", "moonshot", "minimax", "mimo", "opencodego", "stepfun"];
