...
一个开源的浏览器扩展，在 B 站视频页右侧的文摘阅读面板中提供字幕阅读、导出、AI 总结和内容讲解，不用离开视频页面。
...
- 章节按视频结构分段，点击跳播对应位置
- 重点引用（金句）同样带时间戳，点击跳播对应位置

### AI 对话栏

- 围绕字幕内容多轮对话，追问细节或拓展知识点
- 内置预设提示词、模型切换、深度思考开关和历史对话
- 可在输入框粘贴图片发图：截图或复制的图片随消息一起发送，模型可结合画面内容回答（需模型支持图片输入；发送前在本地压缩，单条最多 4 张）
- 播放器内 AI 快捷按钮默认开启，一键生成视频摘要（可在设置中关闭）
- 选中字幕后弹出「解释」卡片，可一键发到对话栏继续追问
- 可选联网搜索（默认关闭）：需要时效性事实或视频没讲到的内容时，模型可自行联网查询，回答里带搜索时间线、来源链接和 `[n]` 内联引用；开启前先在设置里添加搜索平台，推荐 Tavily（下文「设置 API Key」有配置步骤与三家免费额度对比）
- AI 回答中的时间戳同样可点击跳转
- AI 回答里的 ` ```mermaid ` 代码块渲染成流程图或时序图（浅色/深色跟随面板；其他图表类型保留源码并提示）
...
在阅读面板（文摘）右上角点齿轮按钮打开 **设置** 抽屉，Key 只能粘贴到这里的设置输入框中。不要把 Key 发到 AI 对话、项目文件、截图或公开消息中。

添加平台时选择 ModelScope 预设，Base URL 会自动填充。粘贴 Token 后，点模型名称旁的箭头即可拉取可用模型列表，从中选择一个，无需手动填写。想换模型或改用其他平台，随时在设置的「AI 模型平台」中修改。

### 获取搜索平台 API Key（可选）

只在你要用「联网搜索」时才需要，三家选一家即可，**首选 Tavily**：基础搜索每次只扣 1 credit，免费额度是每月 1,000 credits，消耗一眼能对上。三家的免费额度如下（2026-09 查询各自官方页面，实际以官方为准）：

| 平台 | 免费额度 | 备注 |
| --- | --- | --- |
| **Tavily（推荐）** | 每月 1,000 credits | 基础搜索每次扣 1 credit，计费最直观 |
| Exa | 每月 $10 额度 | 搜索请求 $7 / 1,000 次，另计摘要费用 |
| Brave Search | 每月 $5 额度 | 免费计划需要绑定信用卡 |

扩展每次搜索只发一个请求、取回 5 条结果，所以按「单轮最多 5 次搜索」的默认上限，Tavily 的免费额度大致够每月 200 轮联网对话。

到对应平台注册并创建 API Key，然后在设置的「搜索平台」区块点「+ 添加平台」，选预设、粘贴 Key 保存，再点该行的单选钮把它设为当前选用平台。同一区块的「单轮搜索次数上限」默认 5，可调 1–10。

搜索请求由扩展后台代发，搜索平台的 Key 与 AI / 语音识别的 Key 一样只存本机，不随 Chrome 账号同步。开了联网开关但没配置平台时，对话会提示本轮未联网，回答照常给出；模型本身不支持工具调用（function calling）时同样会提示并退回无联网回答。
...
1. 打开一个带字幕的 B 站视频页面。
2. 点击视频播放器下方的 **文摘按钮**，或点击浏览器**工具栏上的扩展图标**——两个入口完全等价，都打开同一个页面内右侧的文摘阅读面板。
3. 阅读带时间戳的字幕；字幕、概览、AI 对话和设置都位于这个页面内文摘面板的标签页中，不需要离开视频页。
4. 切换到 **AI 标签页**，查看 AI 生成的视频摘要，或围绕字幕内容多轮对话；输入行右侧的「联网搜索」开关可让模型联网查证视频之外的信息，开关状态会记住。
5. 选中字幕，获取 AI 内容讲解。
...
- AI 总结、选中文本讲解、翻译和润色。
- AI 对话与选区解释的联网搜索（Tavily / Exa / Brave，需自备对应平台的 Key）。
- AI 对话中粘贴图片发图（需所选模型支持图片输入）。
- 最近字幕、概览和翻译的本地缓存。
...
- `storage` 与 `unlimitedStorage`：保存设置、API Key、对话、字幕、概览和本地缓存；只有非敏感设置通过 Chrome 账号同步。字幕 / 小结 / 概览等缓存族各自只保留最近 3 个视频、不设字节上限，`unlimitedStorage` 即为此声明，避免缓存被 Chrome 常规配额清理。
- `scripting`：在用户当前打开的 B 站视频页注入页面内文摘面板，并在视频、面板和播放器之间同步状态。
...
- `https://www.bilibili.com/*`：在视频页与稍后再看列表页注入文摘按钮和阅读面板。
...
- `https://api.tavily.com/*`、`https://api.exa.ai/*`、`https://api.search.brave.com/*`：仅在你开启联网搜索时，把模型生成的查询词发往你选用的搜索平台并取回结果；这三个域名随扩展静态声明，没配置搜索平台时不会发起任何请求。
- 可选的全域权限（`http://*/*`、`https://*/*`）：只在你保存 AI / 语音识别平台、或点模型名右侧箭头拉取该平台模型列表时申请，用于连接你指定的端点（含本机自部署服务），删除对应平台后回收。
...
首次使用 AI 功能前，需要配置 AI 模型平台。设置入口在阅读面板（文摘）右上角的齿轮按钮，点击打开「设置」抽屉即可看到全部设置项：

1. 打开阅读面板，点击右上角齿轮（设置）
2. 在「AI 模型平台」区域点击「+ 添加平台」
3. 选择平台预设（自动填充 API Base URL 与所属协议），或选择「自定义」手动填写
4. 选择协议（OpenAI / Anthropic）：预设已带好默认值，接入 Anthropic 端点的平台时改选对应协议，Base URL 会跟随联动
5. 填写 API Key 和模型名称（点击模型名称旁的箭头可自动拉取可用模型列表）
6. 点击「测试」验证连接，测试成功后配置自动保存

填写模型时，内置模型目录会为已知模型标注上下文窗口（如 `1M`、`524K`）与「思考」「图片」能力徽标，鼠标悬停可看输出上限与数据版本。目录数据随扩展内置、只在打开编辑弹窗时加载，查不到的模型不显示任何占位，也不影响保存与调用。
...
- 搜索平台：联网搜索选用的平台（Tavily / Exa / Brave）与单轮搜索次数上限（默认 5，可调 1–10）
- 导出 / 笔记属性：下载格式、Frontmatter 字段、自定义属性与正文附加段落，以及是否在笔记正文嵌入 B 站播放器（MarkText 等不渲染 `iframe` 的编辑器可关闭）

### 已支持的平台预设

本扩展支持两种 AI 服务协议：OpenAI（兼容）、Anthropic。协议决定请求格式、鉴权方式与流式解析，设置里选好协议后「测试」与对话、联网搜索都走该协议。内置以下平台预设（除 DeepSeek 默认走 Anthropic 协议外，其余预设默认走 OpenAI 协议；多家预设也登记了各自的 Anthropic 端点，切协议时 Base URL 自动跟随）：
...
| AMD Radeon Cloud | AMD Radeon Cloud（免费） |
| 商汤 SenseNova  | SenseNova 商汤（免费） |
| Ollama（本地部署） | Ollama (本地) |

没有你要用的平台时，选择「自定义」预设，按需选择协议并手动填写该平台的 API Base URL（OpenAI 兼容协议需兼容 `/v1/chat/completions` 规范）。存量平台记录默认按 OpenAI 兼容协议运行，行为与本功能上线前一致。
...
4. 开启「联网搜索」后，模型生成的查询词（选区解释链还会带上选中文本及其上下文字幕）会发送给你在设置中选用的搜索平台（Tavily / Exa / Brave）。搜索请求由扩展后台代发，Key 不出后台；关闭开关后不发起任何搜索请求。
5. 开启「无字幕视频语音识别回退」后，无字幕视频的音频会发送到你选用的语音识别平台（如硅基流动）转写。「本地 Whisper」预设连接本机自部署的转写服务，音频不出本机。不开启回退时，本工具不抓取也不上传任何音频。
6. API Key、对话与最近缓存仅存本地（`chrome.storage.local`，不随账号同步）；非敏感设置（系统提示词、下载格式、Frontmatter 字段等）经 `chrome.storage.sync` 随 Chrome 账号在登录设备间同步。
...
- 首次点箭头会弹出该平台域名的授权请求，需选择「允许」（拒绝后下拉里会提示未授权，再点一次箭头可重试）
...
先确认两处：AI 对话输入行右侧的「联网搜索」开关要处于开启状态，设置里「搜索平台」区块要有一个被选中的平台。模型不支持工具调用（function calling）时，扩展会提示并退回无联网回答；提示额度用尽时，检查搜索平台自己的配额。没配置平台不会中断对话，只会提示本轮未联网。
...
├── chat/                  # 对话运行时与会话存储
├── search/                # 联网搜索（Tavily / Exa / Brave 适配器与执行器）
├── core/                  # 核心状态、运行时、消息处理
...
- 调整 AI 总结的提示词和输出格式
- 新增自定义总结命令或快捷方式
- 修改视频总结的触发方式（自动总结 / 播放器按钮 / 文摘面板手动触发）
- 增加更多翻译语言或自定义总结模板
