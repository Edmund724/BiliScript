...
- 可选联网搜索：需要时效性事实或视频没讲到的内容时，模型可自行联网查询，回答里带搜索时间线、来源链接和 `[n]` 内联引用；默认关闭，需先在设置里添加搜索平台（Tavily / Exa / Brave）
- AI 回答中的时间戳同样可点击跳转
...
只在你要用「联网搜索」时才需要，三家选一家即可：Tavily、Exa 或 Brave Search。到对应平台注册并创建 API Key，然后在设置的「搜索平台」区块点「+ 添加平台」，选预设、粘贴 Key 保存，再点该行的单选钮把它设为当前选用平台。同一区块的「单轮搜索次数上限」默认 5，可调 1–10。
...
联网搜索用的 Tavily / Exa / Brave 各有自己的免费额度与计费规则（Brave 免费计划需要绑定信用卡才拿得到 Key），具体以各平台官方页面为准。不开联网搜索时不产生任何搜索请求。
