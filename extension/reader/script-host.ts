...
// 贴栏宽度下限（script-only-ui 排版定稿）：面板宽度档已定死 380px，可填宽度
// 不足时左缘向左延伸补足。
//
// player-host 整页接管退役后，本模块是 LAYOUT 层唯一的布局调度器
//（rAF 合帧 + 脏检查，思路源自旧 player-host 调度器）——script-host
...
// ui/script-button.ts 头注）；SPA 换页换掉锚点节点的场景由 2s 定时自查
// 覆盖（节拍与拆单源的理由见 REANCHOR_INTERVAL_MS 处注释，与 script-button
...
// 右栏锚点候选（按优先级）。判定规则与覆盖页面见 closeScriptHost 上方注释；
...
// 贴栏宽度下限（定死值 = 380px，script-only-ui 排版定稿）：贴栏面板正常吃掉
...
// 合帧与 ResizeObserver 自带重锚（applyScriptRect 每拍比对换锚），此拍只在
...
// ===== 模块级生命周期状态（open/close 属主，幂等由 openScriptHost 守卫） =====
...
export function closeScriptHost(): void {
  ...
  window.removeEventListener("resize", requestScriptLayout);
  window.removeEventListener("scroll", onScriptScroll);
  document.removeEventListener("scrollend", endScriptScroll);
  endScriptScroll();
  lastSnapshot = null;
  ...
  for (const name of SCRIPT_VARS) {
    readingView.style.removeProperty(SCRIPT_VAR_PREFIX + name);
  }
...
function bindScriptHostListeners(): void {
  ...
  // 事件监听走零参入口 requestScriptLayout：Event 实参由它丢弃，且具名函数
  // 保证 add/remove 拿到同一引用（箭头每次新建，remove 永不命中、监听泄漏）。
  window.addEventListener("resize", requestScriptLayout);
  window.addEventListener("scroll", onScriptScroll, { passive: true });
...
function markScriptScrolling(): void {
  const readingView = document.getElementById(ids.readingView);
  if (!readingView) {
    return;
  }
  ...
  scrollIdleTimer = window.setTimeout(endScriptScroll, SCROLL_IDLE_TIMEOUT_MS);
}

// 收尾：摘标记、停定时器。scrollend 与定时器、close 都走这里，幂等。
function endScriptScroll(): void {
  if (scrollIdleTimer) {
    window.clearTimeout(scrollIdleTimer);
    scrollIdleTimer = 0;
  }
...
// 透传给本帧的 applyScriptRect（同拍复用，不重搜）；事件路径不传 finding，
// 一帧内的重复请求合并为一帧（rAF 只认 fn 引用，不认形参）。
function scheduleScriptLayout(finding?: AnchorFinding): void {
  if (layoutRafId) {
    return;
  }
  layoutRafId = window.requestAnimationFrame(() => runScriptLayout(finding));
}

function runScriptLayout(finding?: AnchorFinding): void {
  layoutRafId = 0;
  ...
  applyScriptRect(finding);
}

// 2s 自查：锚点节点被 B 站换掉（SPA 换页/重渲染）时重锚。自查只做一次
// 搜索（锚点与 rect 一并得到），经 rAF 合帧把结果交给 applyScriptRect 复用
//（一拍一搜：自查路径全拍 findScriptAnchor 恰一次、rect 恰一次），换观察
// 对象的判断收在 applyScriptRect——自查自身不换锚，只触发合帧。
function checkReanchor(): void {
  if (!reanchorTimer) {
    return;
  }
  scheduleScriptLayout(findScriptAnchor() ?? undefined);
}

// ===== 锚点与形态计算 =====

// findScriptAnchor 的返回：命中的锚点与其筛选时量得的 rect（rect 一并给出，
...
function findScriptAnchor(): AnchorFinding {
...
function observeScriptAnchor(anchor: Element | null): void {
...
// 一并量得），跳过 findScriptAnchor 与重复 rect 读。观察对象的换锚判断在
// 此单点收口，两条路径同样过这段。
function applyScriptRect(finding?: AnchorFinding): void {
  const readingView = document.getElementById(ids.readingView);
  if (!readingView) {
    return;
  }

  const found = finding ?? findScriptAnchor();
  const anchor = found?.anchor ?? null;
  if (anchor !== observedAnchor) {
    observeScriptAnchor(anchor);
  }
...
function applyPinnedRect(
  ...
  readingView.removeAttribute(FLOAT_ATTR);
  SCRIPT_VARS.forEach((name, index) => {
    readingView.style.setProperty(SCRIPT_VAR_PREFIX + name, values[index]);
  });
}
