...
- [x] 备好发布者显示名与受监控的支持邮箱：**Edmund Yu** / `edmundyuyue@163.com`
- [x] 定稿 Notes for certification（见 §6）
...
**这一步的残留风险**：扩展**没有任何离线示例数据**（已核实，包内无 fixture、无预置缓存），所以「概览 / AI 对话 / 语音识别」三块在认证环境里完全不可测。政策 §1.3 要求扩展对认证人员可测，存在被判"无法测试"而拒的可能。已拍板：**走路线 1，接受风险，只靠 Notes 说明 bring-your-own-key 架构的必然性**，不在提交前新增示例数据代码。
